...
static int add_clip( char *s, lwClip **clist, int *nclips )
{
   ...
   clip = (lwClip*)calloc( 1, sizeof( lwClip ));
   if ( !clip ) return 0;
   ...
   lwListAdd( (void**)clist, (void*)clip );
...
static int add_tvel( float pos[], float vel[], lwEnvelope **elist, int *nenvs )
{
   ...
   for ( i = 0; i < 3; i++ ) {
      env = (lwEnvelope*)calloc( 1, sizeof( lwEnvelope ));
      key0 = (lwKey*)calloc( 1, sizeof( lwKey ));
      key1 = (lwKey*)calloc( 1, sizeof( lwKey ));
...
static lwTexture *get_texture( char *s )
{
   ...
   tex = (lwTexture*)calloc( 1, sizeof( lwTexture ));
   if ( !tex ) return NULL;
...
lwSurface *lwGetSurface5( FILE *fp, int cksize, lwObject *obj )
{
   ...
   surf =(lwSurface*)calloc( 1, sizeof( lwSurface ));
   if ( !surf ) goto Fail;
   ...
   while ( 1 ) {
      ...
      switch ( id ) {
         ...
         case ID_GLOS:
            surf->glossiness.val = ( float ) log( (float) getU2( fp )) / 20.7944f;
            break;
         ...
         case ID_BTEX:
            ...
            tex = get_texture( s );
            lwListAdd( (void**) &surf->bump.tex, (void*) tex );
            break;

         case ID_CTEX:
            s = (char*)getbytes( fp, sz );
            tex = get_texture( s );
            lwListAdd( (void**)&surf->color.tex,(void*) tex );
            break;

         case ID_DTEX:
            s = (char*)getbytes( fp, sz );
            tex = get_texture( s );
            lwListAdd( (void**)&surf->diffuse.tex,(void*) tex );
            break;

         case ID_LTEX:
            s = (char*)getbytes( fp, sz );
            tex = get_texture( s );
            lwListAdd( (void**)&surf->luminosity.tex,(void*) tex );
            break;

         case ID_RTEX:
            s =(char*) getbytes( fp, sz );
            tex = get_texture( s );
            lwListAdd( (void**)&surf->reflection.val.tex, (void*)tex );
            break;

         case ID_STEX:
            s =(char*) getbytes( fp, sz );
            tex = get_texture( s );
            lwListAdd( (void**)&surf->specularity.tex, (void*)tex );
            break;

         case ID_TTEX:
            s = (char*)getbytes( fp, sz );
            tex = get_texture( s );
            lwListAdd( (void**)&surf->transparency.val.tex, (void*)tex );
            break;
         ...
         case ID_SHDR:
            shdr = (lwPlugin*)calloc( 1, sizeof( lwPlugin ));
            if ( !shdr ) goto Fail;
            shdr->name = (char*) getbytes( fp, sz );
            lwListAdd( (void**)&surf->shader, (void*)shdr );
            surf->nshaders++;
            break;
...
lwObject *lwGetObject5( const char *filename, unsigned int *failID, int *failpos )
{
   ...
   fp = WiiFile::FileOpenForRead( filename );
   if ( !fp ) return NULL;
   ...
   object = (lwObject*)calloc( 1, sizeof( lwObject ));
   if ( !object ) goto Fail;

   layer = (lwLayer*) calloc( 1, sizeof( lwLayer ));
   if ( !layer ) goto Fail;
   object->layer = layer;
   object->nlayers = 1;
   ...
   while ( 1 ) {
      ...
      switch ( id )
      {
         ...
         case ID_SURF:
            ...
            if ( !node ) goto Fail;
            lwListAdd( (void**)&object->surf, (void**)node );
            object->nsurfs++;
            break;
      ...
      if ( (int)formsize <= ftell( fp ) - 8 ) break;
