   ...
   int perpoly )
{
   ...
   vmap = (lwVMap*)calloc( 1, sizeof( lwVMap ));
   if ( !vmap ) {
      free( buf );
      return NULL;
   }
   ...
   vmap->nverts = npts;
   vmap->vindex = (int*) calloc( npts, sizeof( int ));
   if ( !vmap->vindex ) goto Fail;
   if ( perpoly ) {
      vmap->pindex = (int*)calloc( npts, sizeof( int ));
      if ( !vmap->pindex ) goto Fail;
   }

   if ( vmap->dim > 0 ) {
      vmap->val = (float**) calloc( npts, sizeof( float * ));
      if ( !vmap->val ) goto Fail;
      f = (float*) malloc( npts * vmap->dim * sizeof( float ));
      if ( !f ) goto Fail;
      for ( i = 0; i < npts; i++ )
...
int lwGetPointVMaps( lwPointList *point, lwVMap *vmap )
{
   ...
   for ( i = 0; i < point->count; i++ ) {
      if ( point->pt[ i ].nvmaps ) {
         point->pt[ i ].vm = (lwVMapPt*)calloc( point->pt[ i ].nvmaps, sizeof( lwVMapPt ));
...
int lwGetPolyVMaps( lwPolygonList *polygon, lwVMap *vmap )
{
   ...
   for ( i = 0; i < polygon->count; i++ ) {
      for ( j = 0; j < polygon->pol[ i ].nverts; j++ ) {
         ...
         if ( pv->nvmaps ) {
            pv->vm = (lwVMapPt*)calloc( pv->nvmaps, sizeof( lwVMapPt ));
            if ( !pv->vm ) return 0;
            pv->nvmaps = 0;
         }
      }
